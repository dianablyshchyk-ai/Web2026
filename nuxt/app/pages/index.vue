...
<template>
  <div class="bg-gray-200 flex flex-col items-center justify-center min-h-screen p-4 font-sans text-slate-800">
    ...
    <div class="w-full max-w-6xl flex justify-center mb-6 mt-4">
      <div class="flex border border-gray-300 rounded-md overflow-hidden bg-gray-50/50 p-0.5 shadow-sm">
        <NuxtLink to="/" class="flex items-center px-4 py-1.5 bg-white shadow-sm text-gray-700 rounded-sm font-semibold text-xs cursor-default">
    ...
    <div class="flex flex-wrap justify-center gap-6 max-w-6xl w-full items-stretch pb-10">
      <div v-for="item in products" :key="item.title"
        ...
        <div class="p-6 flex flex-col h-full">
          ...
          <p v-if="item.oldPrice" class="text-[12px] text-gray-400 mb-1 leading-tight">
          ...
          <p v-else class="text-[12px] text-transparent mb-1 leading-tight select-none">Placeholder</p>

          <div v-if="item.savings !== '0'" class="self-start bg-green-50 text-green-600 text-[11px] font-bold px-2 py-0.5 rounded mb-4">
